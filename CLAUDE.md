...
There is no build step or lint config. There is a pytest suite under `tests/` (install with `pip install -r requirements-dev.txt`, run with `python -m pytest`) that, without needing a live WebLogic domain:
- monkeypatches `_execute_wlst_script` to capture the generated WLST/Jython source instead of running it, then asserts on that source via the `ast` module (e.g. that user-supplied values can't break out of their string literal — `test_tool_injection.py`/`test_script_injection.py`) or via substring checks for code that's harder to parse generically (`test_remote_filesystem_awareness.py`, `test_create_datasource.py`).
- monkeypatches it to return canned/crafted stdout (including realistic WLST banner noise) to exercise the sentinel-line + JSON parsing and Markdown/JSON rendering on the Python side (`test_core_utilities.py`).
- tests Pydantic input validation and `_handle_error` directly (`test_core_utilities.py`).
- spawns a subprocess to verify `wlst_execute_script`'s opt-in registration gate (`test_execute_script_guardrails.py`).

It does not exercise WLST itself — there is no way to validate the *runtime* behavior of a generated script (e.g. whether `cmo.createJDBCSystemResource(...)` actually works against a real domain) without one. For that, exercise the tool through an MCP client (e.g. Claude Code's `.mcp.json`/`~/.claude.json` config, see README.md) against a real or test WebLogic domain.
...
2. **Utility functions** (`_execute_wlst_script`, `_build_connect_script`, `_build_disconnect_script`, `_handle_error`, `_jython_str_literal`) — the core execution engine:
